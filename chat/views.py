...
def chat_delete_message(request, message_pk):
    """Eliminar mensaje - usuarios pueden eliminar propios, admin puede eliminar todos"""
    try:
        current_user = request.user.username
        is_admin = request.user.is_staff or request.user.username == 'admin1'
        ...
        for msg in TEMPORAL_MESSAGES:
            if str(msg.get('id')) == str(message_pk):
                ...
                msg_owner = msg.get('username')
                
                if current_user == msg_owner or is_admin:
                    msg['is_deleted'] = True
                    msg['deleted_info'] = f"Eliminat per {request.user.username}"
                    return JsonResponse({'success': True, 'message': 'Missatge eliminat'})
                else:
                    return JsonResponse({'success': False, 'error': 'No tens permís per eliminar aquest missatge'})
